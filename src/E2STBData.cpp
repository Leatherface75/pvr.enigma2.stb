...
bool CE2STBData::GetDeviceInfo()
{
  ...
  std::string strXML = E2STBUtils->BackendConnection(strURL);
...
bool CE2STBData::LoadChannels(std::string strServiceReference, std::string strGroupName)
{
  ...
  std::string strXML = E2STBUtils->BackendConnection(strTemp);
...
PVR_ERROR CE2STBData::GetEPGForChannel(ADDON_HANDLE handle, const PVR_CHANNEL &channel, time_t iStart, time_t iEnd)
{
  ...
  std::string strURL = m_strBackendBaseURLWeb + "web/epgservice?sRef="
      + E2STBUtils->URLEncodeInline(myChannel.strServiceReference);
  std::string strXML = E2STBUtils->BackendConnection(strURL);
...
{
  std::string strTemp = "web/moviedelete?sRef=" + E2STBUtils->URLEncodeInline(recinfo.strRecordingId);
...
bool CE2STBData::GetRecordingFromLocation(std::string strRecordingFolder)
{
  ...
  {
    strURL = m_strBackendBaseURLWeb + "web/movielist" + "?dirname=" + E2STBUtils->URLEncodeInline(strRecordingFolder);
  }
  ...
  for (; pNode != NULL; pNode = pNode->NextSiblingElement("e2movie"))
  {
    ...
    {
      iTmp = E2STBUtils->TimeStringToSeconds(strTemp.c_str());
    ...
    {
      strTemp.Format("%sfile?file=%s", m_strBackendBaseURLWeb.c_str(), E2STBUtils->URLEncodeInline(strTemp.c_str()));
...
PVR_ERROR CE2STBData::AddTimer(const PVR_TIMER &timer)
{
  ...
  std::string strTemp = "web/timeradd?sRef="
      + E2STBUtils->URLEncodeInline(strServiceReference) + "&repeated="
      + std::to_string(timer.iWeekdays) + "&begin="
      + std::to_string(timer.startTime) + "&end="
      + std::to_string(timer.endTime) + "&name="
      + E2STBUtils->URLEncodeInline(timer.strTitle) + "&description="
      + E2STBUtils->URLEncodeInline(timer.strSummary) + "&eit="
      + std::to_string(timer.iEpgUid);

  if (!g_strBackendRecordingPath.empty())
  {
    strTemp += "&dirname=&" + E2STBUtils->URLEncodeInline(g_strBackendRecordingPath);
  }
...
PVR_ERROR CE2STBData::DeleteTimer(const PVR_TIMER &timer)
{
  ...
  std::string strTemp = "web/timerdelete?sRef="
      + E2STBUtils->URLEncodeInline(strServiceReference) + "&begin="
      + std::to_string(timer.startTime) + "&end="
...
PVR_ERROR CE2STBData::UpdateTimer(const PVR_TIMER &timer)
{
  ...
  std::string strTemp = "web/timerchange?sRef="
      ...
      + std::to_string(timer.startTime) + "&end="
      + std::to_string(timer.endTime) + "&name="
      + E2STBUtils->URLEncodeInline(timer.strTitle) + "&eventID=&description="
      + E2STBUtils->URLEncodeInline(timer.strSummary) + "&tags=&afterevent=3&eit=0&disabled="
      + std::to_string(iDisabled) + "&justplay=0&repeated="
      + std::to_string(timer.iWeekdays) + "&channelOld="
      + E2STBUtils->URLEncodeInline(strOldServiceReference) + "&beginOld="
